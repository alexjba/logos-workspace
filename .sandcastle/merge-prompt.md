...
- `gh pr view <n> --repo <fork> --json mergeable -q .mergeable`. If `CONFLICTING`, first check whether it is only a lock conflict: `git clone -q --branch <branch> https://github.com/<fork>.git /tmp/lockfix-<repo> && cd /tmp/lockfix-<repo> && git merge origin/<base>`. If every conflicting file is `flake.lock`, or `flake.nix` input `url` lines, resolve it yourself: in `flake.nix` keep both sides' changes and, where both moved the same input, keep the newer rev; then regenerate the lock with `nix flake lock` (evaluation, not a build), `git add flake.nix flake.lock`, commit the merge, `git push` (never force) and re-check `mergeable`. Any other conflicting file is real work: `git merge --abort`, comment on the monorepo issue naming the sub-repo and the files, do NOT open the monorepo PR for this issue, and continue with the next issue; the next cycle's implementer merges the fork default branch into it. Delete `/tmp/lockfix-<repo>` either way.
