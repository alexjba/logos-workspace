...
- **Scope.** `FLEET_LABELS=a,b` (AND) and `FLEET_MILESTONE="..."` select the issues a run works; unset means the installed label (`fleet-smoke`). `FLEET_MAX_PARALLEL=N` caps concurrent issue pipelines. Each pipeline is one issue end to end (implement → review → merge); the loop plans again whenever one finishes, so a finished issue lands without waiting for slower ones, and the planner is told which issues are in flight. `FLEET_MAX_PIPELINES` caps how many pipelines a run starts (default `FLEET_MAX_ITERATIONS` × `FLEET_MAX_PARALLEL`). `FLEET_REPLAN_INTERVAL` (seconds, default 600) re-plans on a timer while a slot is free, so an issue filed mid-run does not wait for the next pipeline to finish.
- **Partial work.** An implementer that leaves acceptance criteria open labels the issue `partial`; the merger lands the branch with `Refs #N` instead of `Closes #N`, comments what remains, and removes the label so the issue is re-planned.
